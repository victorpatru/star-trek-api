// Setting up our dependencies for the project
const express = require('express')
const app = express()
const cors = require('cors')
const PORT = 8000

// Allows us to use a local file to access an API
app.use(cors())

// The data that we are going to use to serve back as a response
const aliens = {
    'humans':{
        'speciesName' : 'Humans',
        'homeworld': 'Earth',
...
// We're serving the index.html file to everyone who gets on the landing page
app.get('/', (req, res) => {
    res.sendFile(__dirname + '/index.html')
})

// Creating the API that returns the information about a species based on the chosen type of species
app.get('/api/:speciesName', (req, res) => {
    const species = req.params.speciesName.toLowerCase()

    if (aliens[species]) {
        res.json(aliens[species])
    } else {
